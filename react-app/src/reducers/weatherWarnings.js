...
export default function weatherWarningsReducer (state = [], action) {
    switch(action.type){
        case 'SETWARNING':
 ...
 */
export function retrieveWarningData(type, filter, startDate, endDate)
{
    ...
    if(data != undefined)
    {
        if(!filter)
            ...
            dispatch(setWarningData(getDataFromPeriod(data,startDate,endDate)));           
    }
